...
def go_settings_tab(header_ui):
    ...
    # === 防风控相关 handler ===
    def update_jitter_enabled(value):
        ConfigDB.insert("jitterEnabled", value)
        return gr.update(value=ConfigDB.get_as_bool("jitterEnabled", True))

    def update_shared_state_enabled(value):
        ConfigDB.insert("sharedStateEnabled", value)
        return gr.update(value=ConfigDB.get_as_bool("sharedStateEnabled", True))

    def update_backoff_base_ms(value):
        try:
        ...
        with gr.Tabs(elem_classes="btb-top-tabs"):
            with gr.Tab("请求"):
                with gr.Column(elem_classes="btb-card btb-layout-card"):
                    ...
                    request_interval_ui = gr.Number(
                        ...
                        minimum=1,
                        step=1,
                        info="两次 create 请求之间的间隔。推荐 500ms（整除 1000ms 可对齐 1 秒盾）。",
                    )
                    create_retry_limit_ui = gr.Number(
                        ...
                        minimum=1,
                        step=1,
                        info="每次 prepare 后最多尝试多少次 create。热场建议 30 次以上。",
                    )
                    create_request_batch_size_ui = gr.Number(
                        label="每次 prepare 后尝试 create 次数",
                        value=buy_defaults.create_request_batch_size,
                        minimum=1,
                        step=1,
                        info="一次 prepare 后连续发多少次 create 后再重新 prepare。推荐 3。",
                    )
                    rate_limit_delay_ms_ui = gr.Number(
                        ...
                        minimum=0,
                        step=1,
                        info="旧版：收到 429 后固定等多久。新版已升级为指数退避（自动翻倍）。",
                    )
                    gr.Markdown("")
                    gr.Markdown("## 支付设置")
            ...
            with gr.Tab("防风控"):
                with gr.Column(elem_classes="btb-card btb-layout-card"):
                    gr.Markdown("### 防风控优化开关")
                    gr.Markdown(
                        "以下优化由 biliTickerBuy-mod 新增，默认全部开启。关闭可回退到上游行为。"
                    )
                    jitter_enabled_ui = gr.Checkbox(
                        label="请求间隔随机抖动 ±30%",
                        value=ConfigDB.get_as_bool("jitterEnabled", True),
                        info="在间隔基础上加随机偏移，防止被 B 站按固定间隔模式检测。",
                    )
                    shared_state_enabled_ui = gr.Checkbox(
                        label="多进程共享速率状态",
                        value=ConfigDB.get_as_bool("sharedStateEnabled", True),
                        info="同账号多进程间通过文件共享请求频率，避免撞 1 秒盾和同时 412。",
                    )
                    gr.Markdown("### 429 指数退避设置")
                    rate_limit_backoff_base_ms_ui = gr.Number(
                        label="退避基数（毫秒）",
                        value=ConfigDB.get_as_int(
                        ...
                        minimum=100,
                        step=100,
                        info="首次 429 后等多久。每次翻倍。默认 1000ms。",
                    )
                    rate_limit_backoff_max_ms_ui = gr.Number(
                        label="退避上限（毫秒）",
                        value=ConfigDB.get_as_int(
                        ...
                        minimum=1000,
                        step=1000,
                        info="退避最多涨到多少。默认 30000ms（30 秒）。",
                    )
                    gr.Markdown("### 已内置的优化")
                    gr.Markdown(
                        """
                        - **kfcTime Cookie 自动清理**：预热后自动删除 
                        - **准备订单防重复门卫**：1.5 秒内不重复 prepare
                        - **1 秒盾对齐**：同账号同场次每秒仅 1 次有效 create
                        - **预热连接**：提前建立 HTTP/2 连接
                        """
                    )
                ...
                with gr.Column(elem_classes="btb-card btb-layout-card"):
                    ...
                    gr.Markdown(
                        ...
                        <div class="mt-3 text-sm leading-7 text-slate-700">
                          <p><strong>怎么填写：</strong>推荐每行填写一个代理地址，也支持逗号分隔。</p>
                          <p><strong>支持格式：</strong><code>http://IP:端口</code>、<code>socks5://IP:端口</code>。</p>
                          <p><strong>双账号分流：</strong>LV2 走 SOCKS5 代理时，在操作抢票页的配置文件里指定 <code>--https-proxys</code> 参数。</p>
                        </div>
                        """
                    )
                    gr.Markdown("## 代理策略")
            ...
            with gr.Tab("通知"):
                with gr.Column(elem_classes="btb-card btb-layout-card"):
                    ...
                    gr.Markdown(
                        """
                        🗨️ 配置后抢票成功会发送通知。
                        > [Server酱<sup>Turbo</sup>](https://sct.ftqq.com/sendkey) | [pushplus](https://www.pushplus.plus/) | [Server酱<sup>3</sup>](https://sc3.ft07.com/) | [ntfy](https://ntfy.sh/) | [Bark](https://bark.day.app/) | MeoW
                        """
                    )
                    gr.Markdown("#### Server酱")
                    serverchan_ui = gr.Textbox(
                        value=ConfigDB.get("serverchanKey") or "",
                        label="Server酱ᵀᵘʳᵇᵒ的SendKey｜输入完成后回车键保存",
                        interactive=True,
                        info="https://sct.ftqq.com/",
                    )
                    serverchan3_ui = gr.Textbox(
                        value=ConfigDB.get("serverchan3ApiUrl") or "",
                        label="Server酱³的API URL｜输入完成后回车键保存",
                        interactive=True,
                    ...
                    pushplus_ui = gr.Textbox(
                        ...
                        label="PushPlus的Token｜输入完成后回车键保存",
                        interactive=True,
                    ...
                    meow_ui = gr.Textbox(
                        ...
                        label="MeoW昵称｜输入完成后回车键保存",
                        interactive=True,
                    )
                    gr.Markdown("#### Ntfy")
                    ntfy_ui = gr.Textbox(
                        value=ConfigDB.get("ntfyUrl") or "",
                        label="Ntfy服务器URL｜输入完成后回车键保存",
                        interactive=True,
            ...
            with gr.Tab("系统"):
                with gr.Column(elem_classes="btb-card btb-layout-card"):
                    ...
                    auto_cleanup_logs_ui = gr.Checkbox(
                        ...
                        info="清理 btb_logs 和 btb_runs 中过旧的内容。",
                    )
    ...
    # === 防风控绑定 ===
    jitter_enabled_ui.change(
        fn=update_jitter_enabled,
        inputs=jitter_enabled_ui,
        outputs=jitter_enabled_ui,
    )
    shared_state_enabled_ui.change(
        fn=update_shared_state_enabled,
        inputs=shared_state_enabled_ui,
        outputs=shared_state_enabled_ui,
    )
    _bind_number_commit(
    ...
    def load_go_settings_configs():
        ...
        return [
            ...
            gr.update(value=buy_defaults.rate_limit_delay_ms),
            ...
            gr.update(value=ConfigDB.get_as_bool("jitterEnabled", True)),
            gr.update(value=ConfigDB.get_as_bool("sharedStateEnabled", True)),
            gr.update(
                value=ConfigDB.get_as_int(
                    "rateLimitBackoffBaseMs", RATE_LIMIT_BACKOFF_BASE_MS
                )
