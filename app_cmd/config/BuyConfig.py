...
class BuyConfig(BasicConfig):
    ...
    prepare_max_retries: int = config_field(
        ...
        runtime="prepare_max_retries",
        db="prepareMaxRetries",
        cli="--prepare-max-retries",
        cast=int,
    )
    ...
    prepare_backoff_base_ms: int = config_field(
        ...
        cli="--prepare-backoff-base-ms",
        cast=int,
    )
    """Base exponential backoff for failed prepare requests (ms). Doubles each consecutive failure."""

    refresh_ttl_seconds: int = config_field(
        30,
        env="BTB_REFRESH_TTL_SECONDS",
        runtime="refresh_ttl_seconds",
        db="refreshTtlSeconds",
        cli="--refresh-ttl-seconds",
        cast=int,
    )
    """Minimum TTL for cached refresh_hot_and_warm results."""
    ...
    aggressive_mode: bool = config_field(
        True,
