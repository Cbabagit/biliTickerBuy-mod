...
class Buy:
    ...
    def start_new_terminal(
        ...
        *,
        ...
        # Clash 多实例：分配实例 ID 和 HTTP 代理端口
        env["BTB_CLASH_INSTANCE"] = str(instance_id)
        if instance_id > 0:
            proxy_port = 17890 + instance_id
...
def buy_stream(config: BuyConfig):
    ...
    # 读取当前进程分配的 Clash 实例 ID
    try:
        _clash_instance = int(os.environ.get("BTB_CLASH_INSTANCE", "0"))
        if _clash_instance > 0:
            logger.info(f"[ClashAuto] 当前进程绑定 Clash 实例 {_clash_instance}")
            # 自动保活：确保子实例已启动
            try:
    ...
    def handle_proxy_failure(
        ...
        *,
        ...
        if delay_seconds is None:
            return
        # 对数缩放冷却报告（代替逐秒循环）
        for remaining in _cooldown_counter(delay_seconds):
            yield emit(
                "state",
                None,
                BuyStreamUpdate(
        ...
        if "412 风控" in summary or "ip" in summary.lower() or "location" in summary.lower():
            ...
            try:
                ...
                if clash_ok:
                    ...
                    logger.warning(f"[ClashAuto] 切换失败: {clash_msg}")
            except Exception as exc:
                logger.warning(f"[ClashAuto] 切换异常: {exc}")
            # 2) 走原代理失败处理
    ...
    # === Clash 节点自动切换初始化 ===
    _clash_available = False
    try:
        _clash_available = _clash_api_available(_clash_instance)
        if _clash_available:
            logger.info(f"[ClashAuto] 实例{_clash_instance} API 可用")
            logger.info(f"[ClashAuto] {_clash_status(_clash_instance)}")
            ranked = _clash_ranked(_clash_instance)
            if ranked:
                for line in ranked.splitlines():
                    logger.info(f"[ClashAuto] {line}")
    ...
    if aggressive_mode:
        ...
        config.proxy_backoff_max_seconds = 120
        proxy_backoff.base_seconds = 10
        proxy_backoff.max_seconds = 120
        proxy_backoff.notification_sent = False
        # 429 恢复窗口缩短到 10s
    ...
    # === 自适应限速 + 抖动 ===
    from util.Constant import REQUEST_JITTER_RATIO, MIN_CREATE_INTERVAL_MS

    if aggressive_mode:
        MIN_CREATE_INTERVAL_MS = 500  # 半秒盾
        REQUEST_JITTER_RATIO = 0.15  # ±15% 更紧

    _jitter_enabled = ConfigDB.get_as_bool("jitterEnabled", True)
    _shared_state_enabled = ConfigDB.get_as_bool("sharedStateEnabled", True)

    def _jittered_interval(base_ms: int) -> float:
        """给间隔加随机抖动，返回秒。关闭抖动则返回原间隔。"""
        if not _jitter_enabled:
            return base_ms / 1000.0
        jitter = base_ms * REQUEST_JITTER_RATIO
        return (base_ms + random.uniform(-jitter, jitter)) / 1000.0

    # === 改进 4: 多进程共享速率状态 ===
    buyer_state_manager = None
    if _shared_state_enabled:
        buyer_state_manager = BuyerProcessState(workspace_dir=os.getcwd())
    # 用 project_id + 购买人 hash 标识同场次
    buyer_info_hash = tickets_info.get("buyer_info", "")
    buyer_key = f"{tickets_info['project_id']}_{hash(str(buyer_info_hash))}"

    # === 改进 2: 准备订单防重复 (Issue #961) ===
    _prepare_in_progress = False
    _last_prepare_ts = 0.0
    _prepare_consecutive_failures = 0
    _prepare_total_retries = 0
    PREPARE_COOLDOWN_SECONDS = 3.0  # 常规：3s
    PREPARE_BACKOFF_MAX_SLEEP = 60.0  # 常规：60s 封顶

    if aggressive_mode:
        PREPARE_COOLDOWN_SECONDS = 0.5  # 激进：0.5s
        PREPARE_BACKOFF_MAX_SLEEP = 15.0  # 激进：15s 封顶

    # refresh_hot_and_warm 缓存
    _last_refresh_ts = 0.0
    _cached_hot_project = None
    ...
    def _can_prepare() -> bool:
        """Prepare 防重复：检查是否允许执行 prepare"""
        nonlocal _prepare_in_progress
        if _prepare_in_progress:
            logger.warning("准备订单被防重复门卫拦截：上一个 prepare 仍在执行")
            return False
        now = time.time()
    ...
    def _prepare_done():
        nonlocal _prepare_in_progress, _last_prepare_ts
        _prepare_in_progress = False
        _last_prepare_ts = time.time()

    def _prepare_backoff_sleep() -> float:
        """Prepare 失败指数退避：3s, 6s, 12s, 24s, 48s, capped 60s"""
        ...
        yield emit(
            "status",
            f"准备订单失败(第{_prepare_consecutive_failures}次连续), 等待 {sleep_s:.0f}s 后重试",
        )
        time.sleep(sleep_s)
        return sleep_s

    def _cooldown_counter(total_seconds: int):
        """对数缩放冷却报告：原始语义要等待 total 秒，在关键刻度才 yield 事件"""
        ...
        if remaining <= 15:
            ...
            step = max(15, remaining // 20)
        while remaining > 0:
            # 逐渐缩小步长：越接近 0 越密集
            if remaining <= 15:
                step = 1
            elif remaining <= 60 and step > 5:
                step = 5
            elif remaining <= 300 and step > 15:
                step = 15
            yield remaining
    ...
    def refresh_hot_and_warm():
        nonlocal is_hot_project, _last_refresh_ts, _cached_hot_project
        now = time.time()
        if _last_refresh_ts > 0 and now - _last_refresh_ts < config.refresh_ttl_seconds:
            # 缓存期内直接返回上次结果
            if _cached_hot_project is not None:
                was_hot = is_hot_project
                is_hot_project = _cached_hot_project
                if _cached_hot_project and not was_hot:
                    tickets_info["is_hot_project"] = True
                    logger.info("预热/复检(缓存): hotProject=True")
                else:
                    logger.debug("预热/复检(缓存跳过)")
                return
        ...
        # === 改进 3: 清理预热引入的 kfcTime cookie ===
        _request.cookieManager.remove_cookie(KFC_COOKIE_NAME, domain=KFC_COOKIE_DOMAIN)
        logger.debug("已清理预热遗留的 kfcTime cookie")
    ...
    while isRunning:
        try:
            ...
            # === 改进 2: Prepare 防重复门卫 ===
            if not _can_prepare():
                yield emit("status", "等待上一轮 prepare 冷却...")
                time.sleep(0.2)
                continue
            # === Prepare 总重试上限检查 ===
            ...
            try:
                ...
                if not order_token:
                    ...
                    _prepare_done()
                    yield from _prepare_backoff_sleep()
                    continue
                # 成功获取 token → 重置退避 + proxy backoff
                _prepare_consecutive_failures = 0
                proxy_backoff.reset()
            finally:
                _prepare_done()
            ...
            while attempt <= effective_retry_limit:
                ...
                while attempt <= batch_end:
                    ...
                    try:
                        ...
                        # 记录本次 create 到共享状态
                        if buyer_state_manager:
                            buyer_state_manager.write_create_attempt(buyer_key)
                        ...
                        if terminal_rule is not None:
                            ...
                            break
                        if err == 100051:
                            # 静默续 token：不打断 create 循环，原地重拿 token
                            try:
                                ...
                                logger.warning(f"[回流优化] 静默续 token 失败: {e}")
                            # 续 token 失败，走常规失败流程
                            token_expired = True
                            break
                        if err == 100034:
                        ...
                        elif not handled_blocked:
                            retry_outcome.set_exception(exc)
                    except BiliRateLimitError as e:
                        retry_outcome.set_exception(e)
                        backoff_msg = (
                            f"{e}（第 {e.consecutive_count} 次，退避 {e.backoff_ms}ms）"
                            if e.consecutive_count > 1
                            else f"{e}，退避 {e.backoff_ms}ms"
                        )
                        yield emit(
                            "attempt",
                            backoff_msg,
                    ...
                    if should_sleep_before_next_attempt:
                        time.sleep(_jittered_interval(request_interval))
