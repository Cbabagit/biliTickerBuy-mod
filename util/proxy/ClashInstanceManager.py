...
def _live_filter_nodes(candidates: list[str], need: int) -> list[str]:
    ...
    alive_all = [(n, d) for n, d in results.items() if d is not None and d <= 500]
    alive = heapq.nsmallest(need, alive_all, key=lambda x: x[1])
    slow_or_dead = [n for n, d in results.items() if d is None or d > 500]

    if len(alive) < need:
        logger.warning(
            f"[ClashInstance] B站 延迟测试: 仅 {len(alive_all)} 个可用 (≤500ms), {len(slow_or_dead)} 个超时/不可达, 可用数不足 {need}"
        )
    else:
        logger.info(
            f"[ClashInstance] B站 延迟测试: {len(alive_all)} 个可用 (≤500ms), {len(slow_or_dead)} 个超时/不可达"
        )
    for n, d in alive[:5]:
