...
class CookieManager:
    ...
    def remove_cookie(self, name, domain=".bilibili.com"):
        """从 cookie 列表中移除指定的 cookie（按 name）"""
        cookies = self.db.get(self._COOKIE_KEY)
        if not cookies:
            return
        filtered = [c for c in cookies if c.get("name") != name]
        if len(filtered) < len(cookies):
            self.db.insert(self._COOKIE_KEY, filtered)
            logger.debug(f"已移除 cookie: {name}")
